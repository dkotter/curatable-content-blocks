...
class NS_Post_Finder {
	...
	<input type="text" size="3" maxlength="3" max="3" value="<%= pos %>" class="position">
	<span><%= title %></span>
	<nav>
	...
	public static function render( $name, $value, $options = array() ) {
		...
		<div class="<?php echo esc_attr( $class ); ?>" data-limit="<?php echo intval( $options['limit'] ); ?>" data-args='<?php echo json_encode( $args ); ?>'>
			...
			<ul class="list">
				...
				if ( $options['outside_links'] && ! empty( $value ) && is_string( $value ) ) {
					$items = explode( ',', $value );
					$i = 1;
					foreach ( $items as $item ) {
						if ( is_numeric( $item ) ) {
							printf(
								'<li data-id="%s">' .
									'<input type="text" size="3" maxlength="3" max="3" value="%s" class="position">' .
									'<span>%s</span>' .
									'<nav>' .
							...
							printf(
								...
								$item,
								intval( $i ),
								esc_html( $outside_item[0] ),
								esc_url( $outside_item[1] )
							);
						}
						$i++;
					}
			...
			<?php if ( $options['outside_links'] ) : ?>
			<div class="outside-search">
				<h4>Enter text and URL for outside links</h4>
				<input type="text" class="outside-text" placeholder="Enter text">
				<input type="url" class="outside-url" placeholder="Enter URL">
				<a href="#" class="add">Add</a>
			</div>
			<?php endif; ?>
