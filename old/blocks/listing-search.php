...
class Emmis_Listing_Search_Content_Block extends Tenup_Content_Block {
	...
	?>
		<input type="hidden" name="tenup_content_blocks[<?php echo esc_attr( $row ); ?>][<?php echo esc_attr( $area ); ?>][<?php echo esc_attr( $column ); ?>][<?php echo esc_attr( $iterator ); ?>][type]" value="listing-search" />
	...
	?>
		<div class="module module-listing-search">
			...
			?>
			<form action="/guides/" method="get" id="business-map-form">
				<input type="text" name="key" id="key" placeholder="Find a great place to dine!">
				<select name="loc" id="loc">
					<option value="">Select Location</option>
					<?php
					$location_sections = get_terms( 'emmis-location', array( 'fields' => 'id=>name', 'parent' => 0, ) );
				...
				<select name="bcat" id="bcat">
					...
					<?php
					$category_sections = get_terms( 'business-category', array( 'fields' => 'id=>name', 'parent' => 0, ) );
					foreach ( $category_sections as $parent_id => $section ) : ?>
						<optgroup label="<?php echo esc_attr( $section ); ?>">
							<?php
							$categories = get_terms( 'business-category', array( 'fields' => 'id=>name', 'parent' => $parent_id ) );
